...
static void print_alloc_free_colorful(const char *func_name)
{
    ...
    {
        fprintf(stderr, "\033[1m\033[32mIn %s, alloc_count: %" PRIu64 ", free_count: %" PRIu64 "\033[0m\n", func_name, g_alloc_count, g_free_count);
    }
    else
    {
        fprintf(stderr, "\033[1m\033[31mIn %s, alloc_count: %" PRIu64 ", free_count: %" PRIu64 "\033[0m\n", func_name, g_alloc_count, g_free_count);
    }
}
int main(int argc, char const *argv[])
{
    ...
    stack_test_1();
#ifdef DEBUG_ALLOC_FREE_COUNT
    print_alloc_free_colorful("stack_test_1");
    g_alloc_count = 0;
    g_free_count = 0;
#endif
